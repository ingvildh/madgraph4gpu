...
)
{
  ...
  const unsigned int block_matrix_rows = 3;
  const unsigned int block_matrix_cols = 6;
  const unsigned int block_rows = 8;
  const unsigned int block_cols = 4;
  ...
  int base_offset_a_row = 0;
    ...
    for (int blockrow=0; blockrow<block_matrix_rows; blockrow++) {
      for (int blockcol=0; blockcol<block_matrix_cols; blockcol++) {
        base_offset_a_row = blockrow*block_rows;
        base_offset_a_col = blockcol*block_cols;
        base_offset_a_block = block_rows*block_cols*(block_matrix_cols*blockrow + blockcol);
        for (int row=0; row<block_rows; row++) {
          for (int col=0; col<block_cols;col++) {
            curr_offset_a_row = base_offset_a_row + row;
            curr_offset_a_col = base_offset_a_col + col;
            curr_offset_a_block = base_offset_a_block + row*block_cols + col;
            A_block[curr_offset_a_block] = A[curr_offset_a_row][curr_offset_a_col]; 
          }
        }
      }
    }
  ...
  multiplyMatrixTensorCore<<<18,32>>>(A_block+32*i, B+32*(i%6), C+64*i);
  cudaDeviceSynchronize();

  // STEP 4: SUM TENSOR CORE RESULT INTO FINAL RESULT MATRIX
  ...
  // STEP 5: COMPLETE MEHELSUM BY USING THE REAL COMPONENT FROM FINAL RESULT MATRIX
